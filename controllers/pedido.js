...
const buscarPorId = async (id) => {
    try {
        ...
        return id ? await pedido.findByPk(id, options) : await pedido.findAll(options);
    } catch (error) {
        throw error;
    }
};
...
const atualizar = async(id, {valorTotal = null, observacoes = null}, produtos = [], enderecoEntrega = []) => {
    ...
    try {
        await pedido.update({valorTotal, observacoes}, {where: {id}, transaction: transacao});

        await produto_pedido.destroy({where: {pedidoId: id}, transaction: transacao});

        for (const item of produtos) { //buscar item na lista produtos
            await produto_pedido.create({
        ...
        await endereco_pedido.update(enderecoEntrega, {where: {pedidoId: id}, transaction: transacao});
        await transacao.commit();
    } catch (error) {
        await transacao.rollback();
        throw error;
...
module.exports = { criar, buscarPorId, atualizar, remover };